...
float votedTemperature(float t1, float t2, float t3) {
  ...
  float valid[3]; int vcount = 0;
  for (int i = 0; i < 3; ++i) {
    float v = inputs[i];
    if (v >= MIN_TEMP && v <= MAX_TEMP) valid[vcount++] = v;
  }
  ...
  if (vcount >= 2) {
    ...
    if (vcount == 3) {
      ...
      if (d01 <= d02 && d01 <= d12) { best_a = valid[0]; best_b = valid[1]; }
      else if (d02 <= d01 && d02 <= d12) { best_a = valid[0]; best_b = valid[2]; }
      else { best_a = valid[1]; best_b = valid[2]; }
    }
    return (best_a + best_b) * 0.5f;
  }

  if (vcount == 1) return valid[0];
